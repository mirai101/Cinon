...
""".format(red('Version'), yellow(__version__)),
    ...
    parser.add_argument('--noprint', action='store_true', default=False,
                        help='cinon will print discovered emails to terminal. It is possible to tell cinon not to print results to terminal with this option.')
    ...
    limit = args.limit
    engine = args.engine
    app = cinon(userAgent, args.proxy)
    ...
    if filename:
        ...
        try:
            ...
            with open(filename, 'w') as out_file:
                out_file.write('<?xml version="1.0" encoding="UTF-8"?><cinon>')
                for email in all_emails:
                    out_file.write('<email>{}</email>'.format(email))
                out_file.write('</cinon>')
